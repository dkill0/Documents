<?php
//for use in displaying the rows on multiple "pages" and other functions
include ('extras.php');

/************************************************************************/
/* ajaxCRUD.class.php	v3.5                                            */
/* ===========================                                          */
/* Copyright (c) 2008 by Sean Dempsey (sean@seandempsey.com)            */
...
class ajaxCRUD{
    ...
    function insertHeader($ajax_file){
        ...
        echo "<script src=\"" . $this->ajaxcrud_root . "js/ajax_functions.js\" type=\"text/javascript\"></script>\n";
        echo "<script src=\"" . $this->ajaxcrud_root . "js/protoculous.js\" type=\"text/javascript\"></script>\n";
        echo "<script src=\"" . $this->ajaxcrud_root . "js/javascript_functions.js\" type=\"text/javascript\"></script>\n";
    ...
    function addButtonToRow($value, $url, $attach_params = ""){
        $this->row_button[] = array(0 => $value, 1 => $url, 2 => $attach_params);
    }
            ...
            if ($_REQUEST['table'] == $this->db_table){
                ...
                if ($string_submitted_values != ''){
                    if (!$this->on_add_specify_primary_key){
                        ...
                        $query = "INSERT INTO $this->db_table($string_fields_with_pk) VALUES (\"$string_submitted_values\")";
                    }
                    $success = qr($query);
	...
	function showTable(){
        ...
        /* call functions based on filtering */
        if ($_REQUEST['ajaxCrudFilterFields'] != ''){
            $filter_fields = $_REQUEST['ajaxCrudFilterFields'];
            $filter_fields_array = explode("|", $filter_fields);

            $filterby_vals    = $_REQUEST['ajaxCrudFilterByVals'];
            $filterby_vals_array = explode("|", $filterby_vals);

            $num_fields = count($filter_fields_array);
            if ($num_fields > 0){
                $count = 0;
                $filter_sql = " WHERE ";
                foreach ($filter_fields_array as $index => $filter_field){
                    $and_word = "";
                    if ($num_fields > $count + 1) $and_word = " AND ";
                    $filter_sql .= "$filter_field LIKE \"%" . $filterby_vals_array[$index] . "%\" $and_word ";

                    $count++;
                }
                $this->addWhereClause($filter_sql);
                $this->filtered_table = true;
            }
        }

        if ($_REQUEST['action'] == 'unfilter'){
        ...
        if (count($this->ajaxFilter_fields) > 0){
            ...
            foreach ($this->ajaxFilter_fields as $filter_field){
                ...
                $top_html .= "<td><b>$display_field</b>: <input type=\"text\" size=\"$textbox_size\" onKeyUp=\"filterTable(this, '$filter_field', '$_SERVER[PHP_SELF]');\"></td>";
            }
            $top_html .= "</tr></table>";
        }
		...
		foreach ($this->category_table_array as $key => $category_table){
            ...
            $order_by = '';
            if ($this->category_field_sort_array[$key] != ''){
                $order_by = " ORDER BY " . $this->category_field_sort_array[$key];
            }
        ...
        if ($row_count > 0){
            ...
            foreach ($this->display_fields as $field){
                ...
                $table_html .= "<th><a href='javascript:;' onClick=\"changeSort('" . $field_name . "', '$this->sort_direction', '" . $_SERVER['PHP_SELF'] . "', $num_ajaxCRUD_tables_instantiated);\" >" . $field . "</a></th>";
            }
            ...
            foreach ($rows as $row){
                ...
                foreach($this->display_fields as $field){
                    ...
                    if ($this->fieldInArray($field, $this->uneditable_fields) || (!$this->ajax_editing)){
                        $table_html .= "<td>";
                        ...
                        if ($this->fieldInArray($field, $this->file_uploads)){
                            ...
                            if ($cell_data != ''){
                                ...
                                $table_html .= "<span id='text_" . $field . $id . "'><a target=\"_new\" href=\"$file_link\">" . $cell_data . "</a> (<a style=\"font-size: 9px;\" href=\"javascript:\" onClick=\"$('file_$field$id').style.display = ''; $('text_$field$id').style.display = 'none'; \">edit</a> <a style=\"font-size: 9px;\" href=\"javascript:\" onClick=\"deleteFile('$field', '$id')\">delete</a>)</span> \n";
                            ...
                            if ($cell_data == ''){
                                $table_html .= "<span id='text_" . $field . $id . "'><a style=\"font-size: 9px;\" href=\"javascript:\" onClick=\"$('file_$field$id').style.display = ''; $('text_$field$id').style.display = 'none'; \">Add File</a></span> \n";
                        ...
                        if (!is_numeric($found_category_index) && $found_category_index == ''){
                            ...
                            if (is_array($this->allowed_values[$field]) && !$this->field_no_dropdown[$field]){
                                ...
                                if (is_array($this->checkbox[$field])){
                                    ...
                                    $field_onKeyPress = "";
                                    if ($this->fieldIsInt($this->getFieldDataType($field)) || $this->fieldIsDecimal($this->getFieldDataType($field))){
                                        $field_onKeyPress = "fn_validateNumeric(this, 'n');";
                                        if ($this->fieldIsDecimal($this->getFieldDataType($field))){
                                            $field_onKeyPress = "fn_validateNumeric(this, 'y');";
                                        }
                                    }
                ...
                if ($this->delete || (count($this->row_button)) > 0){
                    ...
                    if ($this->delete){
                        $table_html .= "<input type=\"button\" class=\"editingSize\" onClick=\"confirmDelete('$this->ajax_file', '$id', '" . $this->db_table . "', '" . $this->db_table_pk ."');\" value=\"delete\" />\n";
                    }
                    ...
                    if (count($this->row_button) > 0){
                        foreach ($this->row_button as $the_row_button){
                            ...
                            $table_html .= "<input type=\"button\" onClick=\"location.href='" . $url . $attach . "'\" class=\"btn editingSize\" value=\"$value\" />\n";
                        }
                    }
        ...
        if ($this->add){
            $add_html .= "<center>\n";
            $add_html .= "   <input type=\"button\" value=\"Add $item\" class=\"btn editingSize\" onClick=\"Effect.SlideDown($('add_form_$this->db_table'));\">\n";
            ...
            if ($this->ajax_add){
                $add_html .= "<input class=\"editingSize\" type=\"button\" onClick=\"
                    $('the_table_div$num_ajaxCRUD_tables_instantiated').innerHTML = '<center><br /><br  /><img src=\'../images/loading.gif\'><br /><br /></center>';
                    //Effect.SlideUp('add_form_$this->db_table');
                    var fields = getFormValues($('add_form_$this->db_table'), '');
            ...
            $add_html .= "</td><td><input style='float: right;' class=\"editingSize\" type=\"button\" onClick=\"Effect.SlideUp('add_form_$this->db_table');\" value=\"Cancel\"></td></tr>\n</table>\n";
    ...
    function fieldIsDecimal($datatype){
        if (stristr($datatype, "decimal") !== FALSE){
            return true;
        }
        return  false;
    }
	...
	function makeAjaxEditor($unique_id, $field_name, $field_value, $type = 'textarea', $field_size = "", $field_text = "", $onKeyPress_function = ""){
        ...
        $return_html .= "<span class=\"editable\" id=\"" . $prefield ."_show\" style=\"cursor: hand;\" onClick=\"
			$('" . $prefield . "_edit').style.display = '';
			$('" . $prefield . "_show').style.display = 'none';
			$('" . $input_name . "').focus();
            \">" . $field_text . "</span>
        <span id=\"" . $prefield ."_edit\" style=\"display: none;\">
            <form style=\"display: inline;\" name=\"form_" . $prefield . "\" id=\"form_" . $prefield . "\" onsubmit=\"
				$('" . $prefield . "_edit').style.display='none';
				$('" . $prefield . "_save').style.display='';
                var req = '" . $this->ajax_file . "?action=update&id=" . $unique_id . "&field=" . $field_name . "&table=" . $this->db_table . "&pk=" . $this->db_table_pk . "&val=' + $('" . $input_name . "').value;
				sndReq(req);
				return false;
			\">";
        ...
        $return_html = "<span class=\"editable\" id=\"" . $prefield . "_show\" style=\"cursor: hand;\" onClick=\"
            ...
            <span style=\"display: none;\" id=\"" . $prefield . "_edit\">
                ...
                <select class=\"editingSize editMode\" id=\"" . $prefield . "\" onChange=\"
                    var selected_index_value = $('" . $prefield . "').value;
                    $('" . $prefield . "_edit').style.display='none';
                    $('" . $prefield . "_save').style.display='';
                    var req = '" . $this->ajax_file . "?action=update&id=" . $unique_id . "&field=" . $field_name . "&table=" . $this->db_table . "&pk=" . $this->db_table_pk . "&dropdown_tbl=" . $dropdown_table . "&val=' + selected_index_value;
                    sndReq(req);
                    return false;
                \">";
	...
	function makeAjaxCheckbox($unique_id, $field_name, $field_value){
        ...
        $return_html .= "<input type=\"checkbox\" $checked onClick=\"
			var " . $prefield . "_value = '';
			if (this.checked){
			...
			var req = '" . $this->ajax_file . "?action=update&id=$unique_id&field=$field_name&table=$this->db_table&pk=$this->db_table_pk&val=' + " . $prefield . "_value;
